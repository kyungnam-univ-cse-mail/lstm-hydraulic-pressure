...
#이상치(Anomalous Data)
def Anomalous(data, percent, m):
    #이상치 검출 및 처리(범위 : 전 10초간 데이터 i-10~i-1)
    window_size = 10 #윈도우 사이즈(10초)
...
def Graph(data, num):
    for col in data.columns[1:]:
        ...
        if(num == 1):
            ...
            plt.savefig('C:/water/result/' + col + '_수정전.jpg', format='jpg')
        elif(num == 2):
            plt.title(col + ' after')
            plt.savefig('C:/water/result/' + col + '_수정후.jpg', format='jpg')
...
if __name__ == "__main__":
    ...
    percent = float(input("이상치로 판단할 범위(%) : "))
    ...
    #파일 생성
    data.to_csv("C:/water/result/result.csv")